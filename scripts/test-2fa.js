...
async function test2FAFunctionality() {
  ...
  try {
    ...
    console.log(`  ✅ CryptoJS encryption working: ${testSecret === decryptedSecret}`);
    ...
    console.log(`  Original codes match decrypted: ${JSON.stringify(backupCodes) === JSON.stringify(decryptedBackupCodes)}`);
    ...
    try {
      ...
      console.log(`  ✅ QR code generated successfully (${qrCodeUrl.length} chars)`);
      console.log(`  QR code format: ${qrCodeUrl.startsWith('data:image/png;base64,') ? 'PNG Base64' : 'Unknown'}`);
    } catch (error) {
