...
testPaths.forEach((path) => {
  ...
  if (match) {
    console.log(
      `  → Locale found: '${match[1]}' (valid: ${isValidLocale(match[1])})`,
    );
  } else {
...
acceptLanguageHeaders.forEach((header) => {
  ...
  console.log(
    `  → First valid locale: ${validLocale || "none (would use default)"}`,
  );
});
...
securityTests.forEach((test) => {
  ...
  console.log(
    `  → Safe locale: '${safeLocale}' ${safeLocale === test.expected ? "✅" : "❌"}`,
  );
});
...
console.log(
  "• Multiple fallback strategies (path → cookie → header → default)",
);
