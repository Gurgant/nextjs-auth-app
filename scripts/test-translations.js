...
for (const locale of SUPPORTED_LOCALES) {
  ...
  try {
    ...
    console.log(`✅ Loaded ${locale}.json (${content.split('\n').length} lines)`);
  } catch (error) {
...
console.log(`📋 Base language (${BASE_LOCALE}) has ${baseKeys.length} translation keys`);
...
for (const locale of SUPPORTED_LOCALES) {
  ...
  const completeness = ((baseKeys.length - missing.length) / baseKeys.length * 100).toFixed(1);
  ...
  console.log(`🌐 ${locale}: ${completeness}% complete (${missing.length} missing keys)`);
}
...
for (const locale in missingKeys) {
  if (missingKeys[locale].length > 0) {
    console.log(`❌ ${locale.toUpperCase()} - Missing ${missingKeys[locale].length} keys:`);
  ...
  for (const key in obj) {
    ...
    if (typeof obj[key] === 'object' && obj[key] !== null) {
      ...
      if (interpolations.length > 0) {
        console.log(`🔧 ${currentPath}: Found interpolations: ${interpolations.join(', ')}`);
      }
...
const completeLocales = Object.values(completenessReport).filter(r => r.completeness === 100).length;
const averageCompleteness = Object.values(completenessReport).reduce((sum, r) => sum + r.completeness, 0) / totalLocales;
...
if (completeLocales === totalLocales) {
  ...
  console.log(`\n⚠️ Action Required:`);
  const incompleteLocales = Object.keys(completenessReport).filter(locale => 
    completenessReport[locale].completeness < 100
  );
  console.log(`   • Complete translations for: ${incompleteLocales.join(', ')}`);
  console.log(`   • Total missing keys: ${Object.values(missingKeys).flat().length}`);
}
