...
function loadTranslations(locale) {
  ...
  try {
    ...
    console.error(
      colorize(`❌ Error loading ${locale}.json: ${error.message}`, "red"),
    );
    return null;
  }
}
...
function validateTranslations() {
  ...
  if (!baseTranslations) {
    console.error(
      colorize("❌ Cannot load base translations. Exiting.", "red"),
    );
    process.exit(1);
  }
  ...
  for (const locale of SUPPORTED_LOCALES) {
    ...
    if (missingKeys.length === 0 && extraKeys.length === 0) {
      console.log(
        colorize(
          `  ✅ ${locale}: All keys present (${localeKeys.length} keys)`,
          "green",
        ),
      );
    } else {
      ...
      if (extraKeys.length > 0) {
        console.log(
          colorize(
            `  ⚠️  ${locale}: Extra ${extraKeys.length} keys:`,
            "yellow",
          ),
        );
  ...
  if (hasErrors) {
    ...
    console.log(
      colorize(`   Total keys per locale: ${baseKeys.length}`, "green"),
    );
  }
