...
async function testEmailConfiguration() {
  ...
  console.log(
    `  ✅ RESEND_API_KEY: ${hasResendKey ? "Set (" + process.env.RESEND_API_KEY.slice(0, 10) + "...)" : "❌ Missing"}`,
  );
  console.log(
    `  ✅ EMAIL_FROM: ${hasEmailFrom ? process.env.EMAIL_FROM : "❌ Missing"}`,
  );
  console.log(
    `  ✅ ENCRYPTION_KEY: ${hasEncryptionKey ? "Set (" + process.env.ENCRYPTION_KEY.slice(0, 5) + "...)" : "❌ Missing"}`,
  );
  console.log(
    `  ✅ NEXTAUTH_SECRET: ${hasNextAuthSecret ? "Set (" + process.env.NEXTAUTH_SECRET.slice(0, 5) + "...)" : "❌ Missing"}`,
  );
  ...
  try {
    ...
    if (error.message.includes("401")) {
      ...
      console.error(
        "  → Permission denied. Check your Resend domain verification",
      );
    } else {
