...
export class CreateOrderCommand extends BaseCommand<CreateOrderInput, Order> {
  ...
  async undo(): Promise<void> {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await eventBus.publish(new OrderCancelledEvent({
...
describe('User Service', () => {
  ...
  it('should handle multiple users', async () => {
    ...
    const results = await Promise.all(
      users.map(u => userService.create(u))
    )
...
// 1. Command
export class CreateReviewCommand extends BaseCommand<CreateReviewInput, Review> {
  ...
  protected async executeImpl(input: CreateReviewInput): Promise<Review> {
    ...
    // Emit event
    await eventBus.publish(new ReviewCreatedEvent({
      reviewId: review.id,
