 ...
 */
export abstract class ChainableBuilder<T, Self extends ChainableBuilder<T, Self>> extends Builder<T> {
  /**
 ...
 */
export abstract class StatefulBuilder<T, State = any> extends ChainableBuilder<T, StatefulBuilder<T, State>> {
...
export type BuilderMethods<T> = {
  [K in keyof T as `with${Capitalize<string & K>}`]: (value: T[K]) => BuilderMethods<T>
} & {
  build(): T
    ...
    protected doBuild(): T {
      ...
      if (validators) {
        for (const [key, validator] of Object.entries(validators)) {
          if (validator && typeof validator === 'function' && !validator(result[key as keyof T])) {
