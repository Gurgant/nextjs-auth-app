...
export class AccountBuilderFactory {
  ...
  createAllProviders(userId: string): {
    ...
    return {
      ...
      credentials: this.create()
        .forUser(userId)
        .credentials(`user_${userId}@example.com`),
    };
  }
...
export const accountBuilders = {
  ...
  credentials: (userId: string, email: string) => {
    return new AccountBuilder().forUser(userId).credentials(email);
  },
...
export class AccountScenarios {
  ...
  static singleOAuth(userId: string, provider = "google"): Account {
    ...
    if (provider === "credentials") {
      return accountBuilders
        .credentials(userId, `${userId}@example.com`)
        .build();
    }
    const builder =
      provider === "github" ? accountBuilders.github : accountBuilders.google;
