...
describe('Navigation Utilities', () => {
  ...
  describe('localizedPath', () => {
    it('should prepend locale to paths', () => {
      ...
      expect(localizedPath('account/settings', 'fr')).toBe('/fr/account/settings')
    })
    ...
    it('should work with array overload', () => {
      expect(localizedPath(['dashboard'], 'en')).toBe('/en/dashboard')
      expect(localizedPath(['account', 'settings'], 'en')).toBe('/en/account/settings')
      expect(localizedPath(['', 'dashboard', ''], 'en')).toBe('/en/dashboard')
    })
  ...
  describe('getPathWithoutLocale', () => {
    it('should extract path without locale', () => {
      ...
      expect(getPathWithoutLocale('/fr/account/settings')).toBe('/account/settings')
    ...
    it('should return original path if no locale pattern', () => {
      ...
      expect(getPathWithoutLocale('/toolong/dashboard')).toBe('/toolong/dashboard')
    })
  ...
  describe('switchLocale', () => {
    it('should switch locale while preserving path', () => {
      ...
      expect(switchLocale('/es/account/settings', 'de')).toBe('/de/account/settings')
  ...
  describe('routes helper', () => {
    ...
    it('should generate error routes with optional query', () => {
      ...
      expect(routes.error('en', 'OAuthAccountNotLinked')).toBe('/en/auth/error?error=OAuthAccountNotLinked')
      expect(routes.error('fr', 'Access Denied')).toBe('/fr/auth/error?error=Access%20Denied')
    })

    it('should generate dynamic routes', () => {
      expect(routes.verifyEmail('en', 'abc123')).toBe('/en/verify-email/abc123')
      expect(routes.linkAccount('en')).toBe('/en/link-account')
      expect(routes.linkAccount('en', 'xyz789')).toBe('/en/link-account/confirm/xyz789')
    })
  })
