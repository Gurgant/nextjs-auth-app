...
export class AnalyticsHandler implements IEventHandler {
  ...
  private processEvent(event: IEvent): void {
    switch (event.type) {
      ...
      case 'security.login_failed':
        ...
        this.incrementCounter(`security.login_failed.${failedLogin.payload.reason}`)
      ...
      case 'system.command_executed':
        ...
        this.updateGauge(`command.${cmdEvent.payload.commandName}.avg_duration`, 
          this.calculateAverageDuration(cmdEvent.payload.commandName, cmdEvent.payload.duration))
        break
        
      case 'system.performance_metric':
        const perfEvent = event as PerformanceMetricEvent
        this.trackMetric(perfEvent.payload.metric, perfEvent.payload.value, 
  ...
  private calculateAverageDuration(commandName: string, newDuration: number): number {
    const metrics = this.metrics.filter(m => 
      m.name === 'command_duration' && m.tags.command === commandName
    )
