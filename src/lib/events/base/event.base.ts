...
export abstract class BaseEvent<TPayload = any> implements IEvent<TPayload> {
   ...
   */
  correlate<T>(EventClass: new (payload: T, metadata?: Partial<EventMetadata>) => IEvent<T>, payload: T): IEvent<T> {
   ...
   */
  static fromJSON<T extends BaseEvent>(data: any, EventClass: new (payload: any, metadata?: Partial<EventMetadata>) => T): T {
