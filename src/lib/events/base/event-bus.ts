...
export class EventBus implements IEventBus {
  ...
  subscribe<T extends IEvent>(
    ...
    if (this.options.enableLogging) {
      console.log(
        `[EventBus] Subscribed to: ${eventType} (${subscription.id})`,
      );
    }
  ...
  subscribeHandler(handler: IEventHandler): string {
    ...
    if (this.options.enableLogging) {
      console.log(
        `[EventBus] Handler subscribed: ${handler.handlerName} to ${handler.eventType}`,
      );
    }
  ...
  unsubscribe(subscriptionId: string): void {
    ...
    if (index !== -1) {
      ...
      if (this.options.enableLogging) {
        console.log(
          `[EventBus] Unsubscribed: ${subscriptionId} from ${subscription.eventType}`,
        );
      }
    }
  }
    ...
    for (let attempt = 1; attempt <= this.options.maxRetries!; attempt++) {
      try {
        ...
        if (attempt < this.options.maxRetries!) {
          if (this.options.enableLogging) {
            console.log(
              `[EventBus] Handler failed, retrying (${attempt}/${this.options.maxRetries}):`,
              {
