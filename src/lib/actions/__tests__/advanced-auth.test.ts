...
jest.mock('@/lib/utils/server-translations', () => ({
  translateError: jest.fn().mockImplementation(async (locale: string, key: string, fallback?: string) => {
  ...
  translateSuccess: jest.fn().mockImplementation(async (locale: string, key: string, fallback?: string) => {
    // Return fallback message if provided, otherwise return a default English message based on key
    if (fallback) return fallback
  ...
  translateCommonError: jest.fn().mockImplementation(async (locale: string, type: string) => {
...
require('@/lib/security').generateSecureToken = jest.fn().mockReturnValue('mock-secure-token')
...
require('@/lib/security').logSecurityEvent = jest.fn().mockResolvedValue(undefined)
require('@/lib/security').getClientIP = jest.fn().mockReturnValue('127.0.0.1')

// Mock implementations for email functions
require('@/lib/email').sendVerificationEmail = jest.fn().mockResolvedValue(true)
require('@/lib/email').sendAccountLinkConfirmation = jest.fn().mockResolvedValue(true)
...
require('@/lib/two-factor').validateBackupCode = jest.fn().mockReturnValue({ valid: true, remainingCodes: [] })
require('@/lib/two-factor').encryptBackupCodes = jest.fn().mockReturnValue(['encrypted1', 'encrypted2'])
require('@/lib/two-factor').generateNewBackupCodes = jest.fn().mockReturnValue(['new1', 'new2'])
require('@/lib/two-factor').isValidTOTPFormat = jest.fn().mockReturnValue(true)
require('@/lib/two-factor').isValidBackupCodeFormat = jest.fn().mockReturnValue(true)
...
describe('Advanced Authentication Actions', () => {
  ...
  describe('sendEmailVerification', () => {
    ...
    it('should return error when user not found', async () => {
      ...
      const result = await sendEmailVerification('nonexistent@example.com', 'en')
