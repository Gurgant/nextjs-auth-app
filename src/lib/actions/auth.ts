...
export async function addPasswordToGoogleUser(formData: FormData, userId: string): Promise<ActionResult> {
  try {
    const data = {
      password: formData.get('password') as string,
      confirmPassword: formData.get('confirmPassword') as string,
...
export async function changeUserPassword(formData: FormData, userId: string): Promise<ActionResult> {
  try {
    const data = {
    ...
    const samePassword = await bcrypt.compare(validatedData.newPassword, user.password)
    if (samePassword) {
      return {
        success: false,
...
export async function migrateUserAccountMetadata(userId: string): Promise<ActionResult> {
  try {
    const user = await prisma.user.findUnique({
      where: { id: userId },
      include: {
    ...
    await prisma.user.update({
      ...
      data: {
        ...
        hasEmailAccount,
        primaryAuthMethod,
        passwordSetAt: user.password && !user.passwordSetAt ? user.createdAt : user.passwordSetAt,
        lastPasswordChange: user.password && !user.lastPasswordChange ? user.createdAt : user.lastPasswordChange,
        lastLoginAt: user.lastLoginAt || new Date()
      }
    })

    console.log('User metadata migrated:', { userId, hasGoogleAccount, hasEmailAccount, primaryAuthMethod })
...
export async function getUserAccountInfo(userId: string) {
  try {
    ...
    return {
      ...
      email: user.email,
      name: user.name,
      hasGoogleAccount: user.hasGoogleAccount !== null ? user.hasGoogleAccount : hasGoogleAccount,
      hasPassword,
      hasEmailAccount: user.hasEmailAccount !== null ? user.hasEmailAccount : hasPassword,
