...
class PerformanceMonitor {
  ...
  async measure<T>(
    ...
    try {
      ...
      if (metric.memoryUsage && metric.memoryUsage.delta.heapUsed > this.memoryLeakThreshold) {
  ...
  measureSync<T>(
    ...
    try {
      ...
      if (duration > 100) { // 100ms threshold for sync operations
  ...
  getStats(): {
    ...
    const slowOperations = this.metrics.filter(m => m.duration > this.slowQueryThreshold).length
    const memoryLeaks = this.metrics.filter(m => 
      m.memoryUsage && m.memoryUsage.delta.heapUsed > this.memoryLeakThreshold
    ).length
    ...
    const avgDbDuration = totalDbQueries > 0
      ? this.dbMetrics.reduce((sum, m) => sum + m.duration, 0) / totalDbQueries
      : 0
    const slowDbQueries = this.dbMetrics.filter(m => m.duration > this.slowQueryThreshold).length
    const dbErrors = this.dbMetrics.filter(m => !m.success).length
    const dbErrorRate = totalDbQueries > 0 ? (dbErrors / totalDbQueries) * 100 : 0
...
export const recordDbQuery = performanceMonitor.recordDatabaseQuery.bind(performanceMonitor)