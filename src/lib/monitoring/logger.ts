...
class Logger {
  ...
  security(event: string, details: Record<string, any>, options: Partial<LogEntry> = {}): void {
  ...
  auth(event: string, userId?: string, details: Record<string, any> = {}, options: Partial<LogEntry> = {}): void {
...
export function createRequestLogger(requestId: string, ipAddress?: string, userAgent?: string) {
  return {
    ...
    error: (message: string, error?: Error, options: Partial<LogEntry> = {}) =>
      logger.error(message, error, { ...options, requestId, ipAddress, userAgent }),
    fatal: (message: string, error?: Error, options: Partial<LogEntry> = {}) =>
      logger.fatal(message, error, { ...options, requestId, ipAddress, userAgent }),
    security: (event: string, details: Record<string, any>, options: Partial<LogEntry> = {}) =>
      logger.security(event, details, { ...options, requestId, ipAddress, userAgent }),
    auth: (event: string, userId?: string, details: Record<string, any> = {}, options: Partial<LogEntry> = {}) =>
      logger.auth(event, userId, details, { ...options, requestId, ipAddress, userAgent }),
    performance: (operation: string, duration: number, options: Partial<LogEntry> = {}) =>
      logger.performance(operation, duration, { ...options, requestId, ipAddress, userAgent })
  }
}
