...
export class ChangePasswordCommand extends BaseCommand<ChangePasswordInput, ActionResponse> {
  ...
  async execute(
    ...
    try {
      ...
      // Hash new password
      const newPasswordHash = await bcrypt.hash(input.newPassword, getBcryptRounds())
      ...
      await eventBus.publish(new PasswordChangedEvent({
        userId: user.id,
        changedAt: new Date(),
  ...
  async undo(): Promise<void> {
    ...
    console.log(`[${this.name}] Restoring previous password for user: ${this.userId}`)
  ...
  async redo(): Promise<void> {
    ...
    console.log(`[${this.name}] Redoing password change for user: ${this.input.userId}`)
