...
export class RegisterUserCommand extends BaseCommand<RegisterUserInput, ActionResponse> {
    ...
    try {
      ...
      if (existingUser) {
        ...
        const error = errorBuilder
          .business.alreadyExists('User', { email: input.email })
        error.log()
        return createErrorResponse(error.getUserMessage())
      }
      
      // Hash password
      const hashedPassword = await bcrypt.hash(input.password, getBcryptRounds())
  ...
  async undo(): Promise<void> {
    ...
    console.log(`[${this.name}] Undoing registration for user: ${this.createdUserId}`)
