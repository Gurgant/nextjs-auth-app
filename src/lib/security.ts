...
export interface SecurityEventData {
  userId: string
  eventType: 'login' | 'failed_login' | 'logout' | 'password_changed' | 'password_reset' | 
            '2fa_enabled' | '2fa_disabled' | '2fa_verified' | '2fa_failed' |
            'account_linked' | 'account_unlinked' | 'email_verified' | 
...
export function generateSecureToken(length: number = 32): string {
  ...
  for (let i = 0; i < length; i++) {
    result += chars.charAt(Math.floor((randomBytes.words[i % 4] >>> (8 * (i % 4))) & 0xFF) % chars.length)
  }
...
export async function checkAccountLockout(userId: string): Promise<boolean> {
  ...
  const lockoutDuration = parseInt(process.env.ACCOUNT_LOCKOUT_DURATION || '15') * 60 * 1000 // Convert to ms
