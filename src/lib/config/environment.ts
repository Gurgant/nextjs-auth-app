...
class EnvironmentValidator {
  ...
  private validateEnvironment(): void {
    ...
    if (this.config.NEXTAUTH_SECRET && this.config.NEXTAUTH_SECRET.length < 32) {
      this.addWarning('NEXTAUTH_SECRET should be at least 32 characters for security')
    }

    // OAuth providers
    this.config.GOOGLE_CLIENT_ID = this.validateRequired('GOOGLE_CLIENT_ID')
    this.config.GOOGLE_CLIENT_SECRET = this.validateRequired('GOOGLE_CLIENT_SECRET')
    ...
    if (this.config.ENCRYPTION_KEY && this.config.ENCRYPTION_KEY.length !== 64) {
      this.addError('ENCRYPTION_KEY must be exactly 64 characters (32 bytes in hex)')
    }

    // Email configuration
    this.config.EMAIL_SERVER_HOST = this.validateRequired('EMAIL_SERVER_HOST')
    this.config.EMAIL_SERVER_PORT = this.validateNumber('EMAIL_SERVER_PORT', 587)
    this.config.EMAIL_SERVER_USER = this.validateRequired('EMAIL_SERVER_USER')
    this.config.EMAIL_SERVER_PASSWORD = this.validateRequired('EMAIL_SERVER_PASSWORD')
    ...
    this.config.ENABLE_EMAIL_VERIFICATION = this.validateBoolean('ENABLE_EMAIL_VERIFICATION', true)
    this.config.ENABLE_ACCOUNT_LINKING = this.validateBoolean('ENABLE_ACCOUNT_LINKING', true)
  ...
  private validateBoolean(key: string, defaultValue: boolean): boolean {
    ...
    this.addWarning(`${key} should be a boolean value (true/false), using default: ${defaultValue}`)
    return defaultValue
  }
  ...
  private validateDatabaseUrl(url: string): void {
    ...
    if (this.config.NODE_ENV === 'production' && !url.includes('sslmode=')) {
      this.addWarning('DATABASE_URL should include SSL configuration for production (sslmode=require)')
    }
  }
  ...
  private validateProductionRequirements(): void {
    ...
    if (!this.config.REDIS_URL) {
      this.addWarning('REDIS_URL not set - consider using Redis for session storage in production')
    }

    if (!this.config.SENTRY_DSN) {
      this.addWarning('SENTRY_DSN not set - consider using error tracking in production')
    }

    // Check security requirements
    if (this.config.LOG_LEVEL === 'debug') {
      this.addWarning('LOG_LEVEL=debug in production may expose sensitive information')
    }
  }
