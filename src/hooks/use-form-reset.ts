  ...
  const resetForm = useCallback(() => {
    try {
      ...
      try {
        ...
        console.error(`[useFormReset:${formName}] Fallback reset also failed:`, fallbackError)
      }
    }
...
export function useMultipleFormReset<T extends Record<string, HTMLFormElement>>(
  ...
  const log = useCallback((message: string, ...args: any[]) => {
    if (debug) {
      console.log(`[useMultipleFormReset:${baseFormName}] ${message}`, ...args)
    }
  }, [debug, baseFormName])
  
  // Reset function for a specific form
  const resetForm = useCallback((formName: keyof T) => {
  ...
  const isFormValid = useCallback((formName: keyof T) => {
    const formRef = formRefs[formName]
    if (formRef?.current) {
