...
describe('useSafeLocale', () => {
  ...
  describe('security scenarios', () => {
    ...
    it('should sanitize XSS attempts', () => {
      mockUseParams.mockReturnValue({ locale: '<script>alert("xss")</script>' })
      const { result } = renderHook(() => useSafeLocale())
      expect(result.current).toBe('en')
    })
...
describe('useSafeLocaleWithOptions', () => {
  ...
  it('should log invalid locales when enabled', () => {
    ...
    renderHook(() => useSafeLocaleWithOptions({
      logInvalid: true,
      componentName: 'TestComponent'
    }))

    // The mock implementation logs to console.warn
    expect(consoleWarnSpy).toHaveBeenCalledWith('[Test] Invalid locale: invalid')
  })
