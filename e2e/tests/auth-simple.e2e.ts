...
test.describe("Basic Authentication Flow", () => {
  ...
  test("should display home page with sign-in options", async ({ page }) => {
    ...
    const hasGoogleButton = await googleButton
      ...
      .isVisible()
      .catch(() => false);
    const hasEmailToggle = await emailToggleButton
      .first()
      .isVisible()
      .catch(() => false);
    const hasGoogleInstead = await googleInsteadButton
      .first()
      .isVisible()
      .catch(() => false);

    // At least one sign-in option should be visible in either state
    const hasAnySignInOption =
      hasGoogleButton || hasEmailToggle || hasGoogleInstead;

    console.log("Sign-in button visibility:", {
      hasGoogleButton,
      hasEmailToggle,
      hasGoogleInstead,
    });
  ...
  test("should show email login form when clicking Sign in with Email", async ({
    page,
  }) => {
  ...
  test("should show error for invalid login", async ({ page }) => {
    ...
    console.log(
      `🔍 Form state - Email input: ${hasEmailInput}, Email button: ${hasEmailButton}`,
    );
    ...
    if (!hasEmailInput) {
      if (hasEmailButton) {
        ...
        throw new Error(
          "Neither email form nor email toggle button is visible",
        );
      }
    }
  ...
  test("should allow successful login with test user", async ({ page }) => {
    ...
    await Promise.race([
      ...
      page
        .waitForSelector("text=/Welcome|Successfully/i", { timeout: 10000 })
        .catch(() => null),
    ]);
...
test.describe("Registration Flow", () => {
  ...
  test("should show error for duplicate email", async ({ page }) => {
    ...
    if (!isDisabled) {
      ...
      await expect(
        page
          .locator("text=/already exists|already registered|email.*taken/i")
          .first(),
  ...
  test("should successfully register new user", async ({ page }) => {
    ...
    await Promise.race([
      page
        .waitForURL(/dashboard|welcome|verify|login|signin/, { timeout: 10000 })
        .catch(() => null),
      page
        .waitForSelector(
          "text=/Success|Registered|Verify|Welcome|Account.*created/i",
          { timeout: 10000 },
        )
        .catch(() => null),
    ]);
    ...
    const hasSuccess =
      (await page
        .locator("text=/Success|Registered|Welcome|Account.*created/i")
        .count()) > 0;
