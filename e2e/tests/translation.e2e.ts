...
test.describe("Translation & i18n Validation", () => {
  ...
  test.describe("Language URL Routing", () => {
    test("should load English content at /en", async ({ page }) => {
      ...
      await expect(
        page.locator(':has-text("Welcome to Our App")').first(),
      ...
      if ((await welcomeElement.count()) > 0) {
        ...
        console.log(
          "✅ Translation-aware test passed: Found expected welcome text",
        );
      } else {
        // Fallback: Look for any welcome-like text
        const hasAnyWelcome =
          (await page
            .locator(':has-text("Welcome"), :has-text("welcome")')
            .count()) > 0;
        expect(hasAnyWelcome).toBeTruthy();
        console.log(
          "✅ Fallback test passed: Found welcome text (translation may differ)",
        );
      }
      ...
      const hasAuthContent =
        (await page
          .locator(
            ':has-text("authentication"), :has-text("sign"), :has-text("login")',
          )
          .count()) > 0;
    ...
    test("should load Spanish content at /es", async ({ page }) => {
      ...
      await expect(
        page.locator(':has-text("Bienvenido a Nuestra App")').first(),
      ).toBeVisible({ timeout: 10000 });
      await expect(
        page.locator(':has-text("Autenticación")').first(),
      ).toBeVisible();
      await expect(
        page.locator(':has-text("Iniciar sesión con Google")').first(),
      ).toBeVisible();
    });
    ...
    test("should load French content at /fr", async ({ page }) => {
      ...
      await expect(
        page.locator(':has-text("Bienvenue dans Notre App")').first(),
      ).toBeVisible({ timeout: 10000 });
      await expect(
        page.locator(':has-text("Authentification")').first(),
      ).toBeVisible();
      await expect(
        page.locator(':has-text("Se connecter avec Google")').first(),
      ).toBeVisible();
    });
    ...
    test("should load German content at /de", async ({ page }) => {
      ...
      await expect(
        page.locator(':has-text("Willkommen in unserer App")').first(),
      ).toBeVisible({ timeout: 10000 });
      await expect(
        page.locator(':has-text("Authentifizierung")').first(),
      ).toBeVisible();
      await expect(
        page.locator(':has-text("Mit Google anmelden")').first(),
      ).toBeVisible();
    });
    ...
    test("should load Italian content at /it", async ({ page }) => {
      ...
      await expect(
        page.locator(':has-text("Benvenuto nella Nostra App")').first(),
      ).toBeVisible({ timeout: 10000 });
      await expect(
        page.locator(':has-text("Autenticazione")').first(),
      ).toBeVisible();
      await expect(
        page.locator(':has-text("Accedi con Google")').first(),
      ).toBeVisible();
    });
  });

  test.describe("Authentication Flow Translations", () => {
    test("should show translated sign-in buttons in different languages", async ({
      page,
    }) => {
      // Test English
      await page.goto("/en");
      await expect(
        page.locator(':has-text("Sign in with Google")').first(),
      ).toBeVisible({ timeout: 10000 });
    ...
    test("should show translated form labels and validation", async ({
      page,
    }) => {
      // Go to registration page in different languages
      await page.goto("/en/register");
      await expect(page.locator(':has-text("Email")').first()).toBeVisible({
        timeout: 10000,
      });
      await expect(page.locator(':has-text("Password")').first()).toBeVisible({
        timeout: 10000,
      });

      await page.goto("/es/register");
      await expect(page.locator(':has-text("Correo")').first()).toBeVisible({
        timeout: 10000,
      });
      await expect(page.locator(':has-text("Contraseña")').first()).toBeVisible(
        { timeout: 10000 },
      );
      ...
      await expect(page.locator(':has-text("passe")').first()).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe("Dashboard Translations with User Context", () => {
    test("should show translated welcome message with user name interpolation", async ({
      page,
    }) => {
      // Login user
      ...
      const hasEmailDisplay =
        (await page.locator(':has-text("test@example.com")').count()) > 0;
  ...
  test.describe("Language Switcher Component", () => {
    test("should have language selector in navigation", async ({ page }) => {
      ...
      const languageSelector = page.locator(
        '[data-testid="language-selector"], .language-selector, :has-text("English"), :has-text("Language")',
      );
    ...
    test("should persist selected language across navigation", async ({
      page,
    }) => {
      // Start with Spanish
      await page.goto("/es");
  ...
  test.describe("Error Message Translations", () => {
    ...
    test("should show translated authentication errors", async ({ page }) => {
      ...
      const stayedOnLogin =
        page.url().includes("/") && !page.url().includes("/dashboard");
      ...
      const stayedOnSpanishLogin =
        page.url().includes("/es") && !page.url().includes("/dashboard");
  ...
  test.describe("Date and Number Formatting", () => {
    test("should format dates according to locale", async ({ page }) => {
      ...
      const hasDateInfo =
        (await page.locator("text=/\\d{1,2}\\/\\d{1,2}\\/\\d{4}/").count()) >
          0 ||
  ...
  test.describe("Complete User Journey in Multiple Languages", () => {
    test("should complete full auth flow in Spanish", async ({ page }) => {
      ...
      await expect(
        page.locator('h1:has-text("Bienvenido")').first(),
      ).toBeVisible({ timeout: 10000 });
      ...
      const hasEmailButton = await emailButton
        .first()
        .isVisible()
        .catch(() => false);

      console.log(
        `🔍 Spanish form state - Email input: ${hasEmailInput}, Email button: ${hasEmailButton}`,
      );
      ...
      if (!hasEmailInput) {
        if (hasEmailButton) {
          ...
          console.log(
            `❌ SPANISH DEBUG - Buttons: ${allButtons}, Inputs: ${allInputs}`,
          );
          console.log(
            `❌ Spanish page content: ${bodyText.substring(0, 500)}...`,
          );

          throw new Error(
            "Neither email form nor email toggle button is visible on Spanish page",
          );
        }
      }
    ...
    test("should complete full auth flow in French", async ({ page }) => {
      ...
      await expect(
        page.locator('h1:has-text("Bienvenue")').first(),
      ).toBeVisible({ timeout: 10000 });
      ...
      console.log(
        `🔍 French form state - Email input: ${hasEmailInput}, Email button: ${hasEmailButton}`,
      );
      ...
      if (!hasEmailInput) {
        if (hasEmailButton) {
          ...
          console.log(
            `❌ FRENCH DEBUG - Buttons: ${allButtons}, Inputs: ${allInputs}`,
          );
          console.log(
            `❌ French page content: ${bodyText.substring(0, 500)}...`,
          );

          throw new Error(
            "Neither email form nor email toggle button is visible on French page",
          );
        }
      }
