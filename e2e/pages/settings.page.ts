...
export class SettingsPage extends BasePage {
  ...
  async uploadAvatar(filePath: string) {
    await this.page.setInputFiles(this.selectors.profile.avatarUpload, filePath)
   ...
   */
  async getProfileUpdateResult(): Promise<{ success: boolean; message?: string }> {
    const success = await this.elementExists(this.selectors.profile.successMessage)
  ...
  async changePassword(currentPassword: string, newPassword: string) {
    await this.fillField(this.selectors.security.currentPasswordInput, currentPassword)
    await this.fillField(this.selectors.security.newPasswordInput, newPassword)
    await this.fillField(this.selectors.security.confirmPasswordInput, newPassword)
  ...
  async toggleTwoFactor(enable: boolean) {
    const isEnabled = await this.page.isChecked(this.selectors.security.twoFactorToggle)
    ...
    if (isEnabled !== enable) {
      ...
      if (enable) {
        // Wait for QR code
        await this.page.waitForSelector(this.selectors.security.qrCode, { timeout: 5000 })
      }
    }
  }
  ...
  async getActiveSessionsCount(): Promise<number> {
    const sessions = await this.page.locator(`${this.selectors.security.sessionsList} li`).count()
    return sessions
  }
  ...
  async updatePreferences(preferences: {
    ...
    if (preferences.language) {
      await this.page.selectOption(this.selectors.preferences.languageSelect, preferences.language)
    }
    
    if (preferences.timezone) {
      await this.page.selectOption(this.selectors.preferences.timezoneSelect, preferences.timezone)
    }
    
    if (preferences.darkMode !== undefined) {
      const isDark = await this.page.isChecked(this.selectors.preferences.themeToggle)
    ...
    if (preferences.notifications !== undefined) {
      const isEnabled = await this.page.isChecked(this.selectors.preferences.notificationsToggle)
    ...
    if (preferences.emailNotifications !== undefined) {
      const isEnabled = await this.page.isChecked(this.selectors.preferences.emailNotificationsToggle)
      if (isEnabled !== preferences.emailNotifications) {
        await this.page.click(this.selectors.preferences.emailNotificationsToggle)
      }
    }
    
    if (preferences.marketingEmails !== undefined) {
      const isEnabled = await this.page.isChecked(this.selectors.preferences.marketingToggle)
  ...
  async updatePrivacySettings(settings: {
    ...
    if (settings.publicProfile !== undefined) {
      const isPublic = await this.page.isChecked(this.selectors.privacy.publicProfileToggle)
    ...
    if (settings.showEmail !== undefined) {
      const showEmail = await this.page.isChecked(this.selectors.privacy.showEmailToggle)
  ...
  async deleteAccount(confirmText: string = 'DELETE') {
    ...
    // Type confirmation
    await this.fillField(this.selectors.privacy.confirmDeleteInput, confirmText)
  ...
  async isSettingsPageDisplayed(): Promise<boolean> {
    return await this.elementExists(this.selectors.pageTitle) &&
  ...
  async waitForSettingsLoad() {
    await this.waitForPageLoad()
    await this.page.waitForSelector(this.selectors.profile.section, { state: 'visible' })
  }
