...
export class LoginPage extends BasePage {
  ...
  async gotoWithRecovery() {
    console.log(
      "🔄 Starting post-logout login page navigation with recovery...",
    );
  ...
  async performLogout() {
    ...
    try {
      ...
      const signOutButton = this.page.locator(
        'button:has-text("Sign out"), button:has-text("Logout"), button:has-text("Log out"), [data-testid*="signout"], [data-testid*="logout"]',
      );
      ...
      const hasSignInOptions =
        (await this.elementExists(this.selectors.emailSignInButton)) ||
        (await this.elementExists(this.selectors.googleButton)) ||
        (await this.page
          .locator(
            ':has-text("Sign in"), :has-text("Login"), :has-text("Welcome to")',
          )
          .count()) > 0;
      ...
      if (hasSignInOptions) {
        ...
        console.log(
          "⚠️ Logout verification unclear - checking for authenticated indicators...",
        );
        const hasWelcomeBack =
          (await this.page
            .locator(
              ':has-text("Welcome back"), :has-text("Dashboard"), :has-text("Sign out")',
            )
            .count()) > 0;
        if (hasWelcomeBack) {
          console.log(
            "❌ Still appears to be authenticated - logout may have failed",
          );
          throw new Error(
            "Logout verification failed - still shows authenticated content",
          );
        } else {
  ...
  async ensureEmailFormVisible() {
    ...
    const emailInputVisible = await this.elementExists(
  ...
  async fillLoginForm(email: string, password: string) {
    ...
    if (!emailInputVisible) {
      console.log(
        "⚠️ No toggle button found, trying alternative approaches...",
      );
      ...
      while (!emailInputVisible && retries < maxRetries) {
        retries++;
        console.log(
          `⏳ Retry ${retries}/${maxRetries}: Waiting for email input...`,
        );
  ...
  async login(email: string, password: string, rememberMe: boolean = false) {
    ...
    await this.page
      .waitForSelector('[data-testid="session-loading"]', { timeout: 2000 })
      .catch(() => null);
    await this.page
      .waitForSelector('[data-testid="session-loading"]', {
        state: "detached",
        timeout: 15000,
      })
      .catch(() => null);
    ...
    await Promise.race([
      ...
      this.page
        ...
        .catch(() => null),
      this.page
        .waitForSelector("text=Welcome", { timeout: 10000 })
        .catch(() => null),
      this.page
        .waitForSelector(this.selectors.twoFactorInput, { timeout: 10000 })
        .catch(() => null),
  ...
  async waitForLoginComplete() {
    await Promise.race([
      ...
      this.page.waitForSelector(this.selectors.twoFactorInput, {
        timeout: 10000,
      }),
    ]);
  }
  ...
  async assertLoginPageDisplayed() {
    ...
    const emailSignInButtonVisible = await this.elementExists(
      this.selectors.emailSignInButton,
    );
    const googleButtonVisible = await this.elementExists(
      this.selectors.googleButton,
    );
    ...
    const passwordInputVisible = await this.elementExists(
    ...
    const hasSignInText =
      (await this.page
        .locator(':has-text("Sign in"), :has-text("Login"), :has-text("Auth")')
        .count()) > 0;
    const hasGoogleText =
      (await this.page.locator(':has-text("Google")').count()) > 0;
    const hasEmailText =
      (await this.page.locator(':has-text("Email")').count()) > 0;
    ...
    const hasWelcomeText =
      (await this.page
        .locator(
          ':has-text("Welcome"), :has-text("Bienvenu"), :has-text("Bienven")',
        )
        .count()) > 0;

    if (isHomePage && hasWelcomeText) {
      console.log(
        "✅ Login page detected: On home page with welcome text (not authenticated)",
      );
      return;
    }
   ...
   */
  async goToDashboard() {
    const dashboardButton = this.page.locator(
    ...
    await Promise.race([
      ...
      if (
        ...
        throw new Error(
          `Failed to navigate to dashboard. Current URL: ${currentUrl}`,
        );
      }
    });
  }
  ...
  async assertRedirectedToDashboard() {
    ...
    if (!isDashboardUrl) {
      throw new Error(
        `Expected to be redirected to dashboard/admin, but current URL is: ${currentUrl}`,
      );
    }
  }
  ...
  async getValidationErrors(): Promise<Record<string, string>> {
    ...
    const emailError = await this.page
      .locator(`${this.selectors.emailInput} ~ .error-message`)
      .textContent();
    if (emailError) errors.email = emailError;

    // Check password field error
    const passwordError = await this.page
      .locator(`${this.selectors.passwordInput} ~ .error-message`)
      .textContent();
