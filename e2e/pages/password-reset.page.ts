...
export class PasswordResetPage extends BasePage {
  ...
  async waitForResetResponse() {
    await Promise.race([
      this.page.waitForSelector(this.selectors.request.successMessage, {
        timeout: 10000,
      }),
      this.page.waitForSelector(this.selectors.request.errorMessage, {
        timeout: 10000,
      }),
      this.page.waitForSelector(this.selectors.request.emailSentIcon, {
        timeout: 10000,
      }),
    ]);
  }
  ...
  async isResetEmailSent(): Promise<boolean> {
    return (
      (await this.elementExists(this.selectors.request.successMessage)) ||
  ...
  async resetPasswordWithToken(token: string, newPassword: string) {
    ...
    await this.fillField(
      this.selectors.confirmation.passwordInput,
      newPassword,
    );
    await this.fillField(
      this.selectors.confirmation.confirmPasswordInput,
      newPassword,
    );

    // Check password strength if indicator exists
    if (
      await this.elementExists(this.selectors.confirmation.passwordStrength)
    ) {
  ...
  async waitForResetConfirmationResponse() {
    await Promise.race([
      this.page.waitForSelector(this.selectors.confirmation.successMessage, {
        timeout: 10000,
      }),
      this.page.waitForSelector(this.selectors.confirmation.errorMessage, {
        timeout: 10000,
      }),
  ...
  async isPasswordResetSuccessful(): Promise<boolean> {
    return (
      (await this.elementExists(this.selectors.confirmation.successMessage)) ||
  ...
  async getResetConfirmationError(): Promise<string | null> {
    ...
    if (
      await this.elementExists(this.selectors.confirmation.expiredTokenMessage)
    ) {
  ...
  async isTokenExpired(): Promise<boolean> {
    return (
      (await this.elementExists(
        this.selectors.confirmation.expiredTokenMessage,
      )) ||
  ...
  async getPasswordStrength(): Promise<"weak" | "medium" | "strong" | null> {
    ...
    const strengthText = await this.getText(
      this.selectors.confirmation.passwordStrength,
    );
  ...
  async checkPasswordRequirements(): Promise<{
    ...
    if (
      !(await this.elementExists(
        this.selectors.confirmation.passwordRequirements,
      ))
    ) {
      // If no requirements shown, assume all are met
      return Object.fromEntries(
        Object.keys(requirements).map((k) => [k, true]),
    ...
    for (const indicator of indicators) {
      ...
      const isValid =
        (await indicator.getAttribute("data-valid")) === "true" ||
  ...
  async isOnRequestPage(): Promise<boolean> {
    return (
      (await this.elementExists(this.selectors.request.emailInput)) &&
  ...
  async isOnConfirmationPage(): Promise<boolean> {
    return (
      (await this.elementExists(this.selectors.confirmation.passwordInput)) &&
      (await this.elementExists(
        this.selectors.confirmation.confirmPasswordInput,
      ))
    );
