...
> {
  ...
  async execute(input: RegisterInput): Promise<RegisterOutput> {
    ...
    try {
      ...
      this.output = createSuccessResponse("User registered successfully", {
        userId: user.id,
      });
...
export class CommandBus {
  ...
  register<T extends ICommand<any, any>>(commandClass: new () => T): void {
...
export class ErrorHandler {
  static handle(error: unknown): ActionResponse {
    ...
    if (error instanceof BaseError) {
      ...
      return createErrorResponse(error.message, error.details?.errors);
    }
