...
jest.mock('@/config/i18n', () => ({
  ...
  DEFAULT_LOCALE: 'en',
  isValidLocale: (locale: string) => ['en', 'es', 'fr', 'it', 'de'].includes(locale),
...
describe('Secure Middleware', () => {
  ...
  describe('Locale Extraction', () => {
    ...
    it('should handle invalid locale attempts', () => {
      ...
      try {
        ...
        const request = new NextRequest(new URL('http://localhost/xx/dashboard'))
        const response = middleware(request)
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle paths with hash fragments', () => {
      ...
      expect(response.headers.get('location')).toBe('http://localhost/en/docs#section-1')
    })
  })
})